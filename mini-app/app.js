...
(function () {
  ...
  if (scroller && heroFull && bgGradient && calendarRow && mascot && healthScore
    ...
    // "Ускоренная" дистанция (collapseDistance * COLLAPSE_SPEED), на которой
    // ВИЗУАЛЬНЫЙ progress (маскот/score/прозрачности второстепенного
    // контента) реально доезжает до 1 — см. update(). Кнопка/Pinned этот
    // ускоренный progress НЕ используют (см. ctaEase в update()) — только
    // сам .hero-full физически прилипает лишь при полном (не ускоренном)
    // collapseDistance, и до этого момента его непрозрачный фон физически
    // ещё шире компактной плашки; если бы кнопка успевала доехать до
    // компактной позиции раньше, она на это время скрылась бы под ещё не
    // сжавшейся плашкой.
    let visualCollapseDistance = 1;
    ...
    const measure = () => {
      ...
      visualCollapseDistance = Math.max(collapseDistance * COLLAPSE_SPEED, 1);
    ...
    const update = () => {
      ...
      // Визуальный прогресс (что показываем — маскот/score/прозрачности)
      // намеренно "быстрее" физического порога прилипания .hero-full (что
      // физически происходит со скроллом) — см. visualCollapseDistance
      // (= collapseDistance * COLLAPSE_SPEED, посчитан в measure()).
      // heroTop ниже считается ОТДЕЛЬНО, напрямую от
      // scrollTop/collapseDistance (без ускорения), поэтому остаётся
      // физически верным независимо от того, что показывает progress.
      // Кнопка/Pinned (см. ctaEase дальше) НАМЕРЕННО этот ускоренный progress
      // не используют — см. подробное объяснение там.
      const progress = Math.min(Math.max(scrollTop / visualCollapseDistance, 0), 1);
      ...
      // контейнера).
      // Важно: ease здесь считаем от ФИЗИЧЕСКОГО progress (ctaProgress,
      // завязан на настоящую collapseDistance), а НЕ от ускоренного progress
      // выше — и это критично по ДВУМ независимым причинам:
      //
      // 1) .pinned двигают не напрямую, а схлопыванием высоты .ctaSpacer, а
      //    у высоты есть жёсткий пол в 0 — .pinned физически не может
      //    обогнать обычный 1:1 скролл. Если бы кнопка доезжала до
      //    компактной точки быстрее, чем позволяет этот пол, между ними на
      //    время открывался бы зазор.
      // 2) Сам .hero-full — это ЖИВАЯ, физически большая (fullHeight)
      //    sticky-плашка с непрозрачным фоном и z-index:10 НАД кнопкой; она
      //    визуально сжимается до компактной плашки ТОЛЬКО в момент, когда
      //    реально прилипает — то есть строго при scrollTop=collapseDistance
      //    (см. вывод формулы collapseDistance выше). До этого момента её
      //    нижний край физически ниже, чем компактная позиция контента
      //    внутри неё. Если кнопка (через ускоренный ease) доедет до своей
      //    компактной цели РАНЬШЕ, чем .hero-full реально прилипнет, она на
      //    это время окажется ПОД ещё физически большой, непрозрачной
      //    плашкой — и просто исчезнет с экрана до срока.
      //
      // Обе причины требуют одного и того же: скорость кнопки не должна
      // опережать физический (не ускоренный COLLAPSE_SPEED) скролл сильнее,
      // чем на финальном отрезке перед самым прилипанием. Поэтому кнопка/
      ...
      // ценой того, что видимый зазор между кнопкой и ещё не погасшим
      // советом на время немного увеличивается (см. cardsProgress/
      // adviceProgress выше — они гаснут заведомо раньше, чем кнопка успеет
      // до них доехать, поэтому наезда на видимый контент не происходит).
